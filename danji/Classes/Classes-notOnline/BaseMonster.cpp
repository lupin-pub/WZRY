...
void BaseMonster::Home()//回城函数
{
	...
	//this->setPosition(200, 125);
	//this->setPosition(205, 120);
	//this->setPosition(200, 115);
	//this->setPosition(195, 120);
...
void BaseMonster::hpSchedule(float dt)//1.0f  给英雄加上这个函数！
{
	...
	GameManager::getInstance()->MONEY = GameManager::getInstance()->MONEY + 0.2;
}
...
void BaseMonster::checkNearestMonster()
{
	...
	if (this->getMonsterColor() == 1)
	{
		...
		for (int i = 0; i < monsterBlueVector.size(); i++)
		{
			...
			monsterDistance = this->getPosition().getDistance(monster->getPosition());//去掉了getParent() 加了baseSprite 应该是这样吧虚虚虚
...
void BaseMonster::checkNearestTower()
{
	...
	if (this->getMonsterColor() == 1)
	{
		...
		for (int i = 0; i < towerBlueVector.size(); i++)
		{
			...
			towerDistance = this->getPosition().getDistance(tower->getPosition());//这才是正解！
	...
	else
	{
		...
		for (int i = 0; i < towerRedVector.size(); i++)
		{
			...
			towerDistance = this->getPosition().getDistance(tower->getPosition());
...
void BaseMonster::death()
{
	...
	//}
		...
		//this->stopAllActions();
		this->unschedule(schedule_selector(BaseMonster::shoot));//shoot
...
void BaseMonster::shoot(float dt)//每隔多久shoot一次 0.5
{
	...
	else if (currMinTower < currMinMonster&&nearestTower != NULL)
	{
		...
		{
			SpriteFrameCache::getInstance()->addSpriteFramesWithFile("GameItem/Red/LongRangeSoldier/animation.plist", "GameItem/Blue/LongRangeSoldier/animation.png");
			AnimationCache::getInstance()->addAnimationsWithFile("GameItem/Red/LongRangeSoldier/animation.plist");
			auto cache = AnimationCache::getInstance();
			auto animation1 = cache->getAnimation("AttactDown");
			animation1->setLoops(1);
...
void BaseMonster::removeBullet()//攻击！  保证打中
{
	...
	if(currMinMonster <=currMinTower&&nearestMonster!=NULL)
	{
	...
	else if (currMinTower < currMinMonster&&currMinTower!=NULL)
	{
...
void BaseMonster::relive()
{
	setState(stateNone);
	setCurrHp(getMaxHp());
	setHpPercentage(100);
	setPosition(Point(200,120));//回城

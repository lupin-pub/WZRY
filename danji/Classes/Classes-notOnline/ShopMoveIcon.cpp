...
bool ShopMoveIcon::initMoveIcon(int type)
{
	...
	String* fileName = String::createWithFormat("ShopScene/Move%d.jpg", type);
	icon = Sprite::create(fileName->getCString());
	icon->setScale(0.3);

	//icon = Sprite::createWithSpriteFrameName(String::createWithFormat("ShopScene/Move_%d.jpg", type)->getCString());
	addChild(icon);

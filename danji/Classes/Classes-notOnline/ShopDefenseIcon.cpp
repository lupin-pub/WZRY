...
bool ShopDefenseIcon::initDefenseIcon(int type)
{
	...
	String* fileName = String::createWithFormat("ShopScene/Defense%d.jpg", type);
	icon = Sprite::create(fileName->getCString());
	icon->setScale(0.3);

	//icon = Sprite::createWithSpriteFrameName(String::createWithFormat("ShopScene/Defense_%d.jpg", type)->getCString());
	addChild(icon);

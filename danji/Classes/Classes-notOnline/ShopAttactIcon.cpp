...
bool ShopAttactIcon::initAttactIcon(int type)
{
	...
	String* fileName = String::createWithFormat("ShopScene/Attact%d.jpg", type);
	icon = Sprite::create(fileName->getCString());
	icon->setScale(0.3);

	//icon = Sprite::createWithSpriteFrameName(String::createWithFormat("ShopScene/Attact_%d.jpg", type)->getCString());
	addChild(icon);

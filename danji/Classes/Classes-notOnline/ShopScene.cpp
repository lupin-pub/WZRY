		...
		if (target->getMoney() <= instance->MONEY) {
			...
			if (tag == Dj) {
				...
				if (shopIconsAttact.empty()) {
					...
					int index = lastIcon -> type;//?????
					lastIcon->setPosition(Director::getInstance()->convertToGL(Vec2(iconX[index - 1], iconY[0])));//?????????????????????????
		...
		if (target->getMoney() <= instance->MONEY) {
			...
			if (tag == Dj) {
				...
				if (shopIconsDefense.empty()) {
					...
					int index = lastIcon->type;//?????
					lastIcon->setPosition(Director::getInstance()->convertToGL(Vec2(iconX[index - 1], iconY[1])));//?????????????????????????
		...
		if (target->getMoney() <= instance->MONEY) {
			...
			if (tag == Dj) {
				...
				if (shopIconsMove.empty()) {
					...
					int index = lastIcon->type;//?????
					lastIcon->setPosition(Director::getInstance()->convertToGL(Vec2(iconX[index - 1], iconY[2])));//?????????????????????????
		...
		if (target->getMoney() <= instance->MONEY) {
			...
			if (tag == Dj) {
				...
				if (shopIconsSpell.empty()) {
					...
					int index = lastIcon->type;//?????
					lastIcon->setPosition(Director::getInstance()->convertToGL(Vec2(iconX[index - 1], iconY[4])));//?????????????????????????
		...
		if (target->getMoney() <= instance->MONEY) {
			...
			if (tag == Dj) {
				...
				if (shopIconsPlayingWild.empty()) {
					...
					int index = lastIcon->type;//?????
					lastIcon->setPosition(Director::getInstance()->convertToGL(Vec2(iconX[index - 1], iconY[3])));//?????????????????????????
...
void ShopScene::initAttactPicture()
{
	...
	addChild(shopIcon1);
	//shopIconsAttact.pushBack(shopIcon1);i
...
void ShopScene::initDefensePicture()
{
	...
	auto shopIcon4 = ShopDefenseIcon::createDefenseIcon(3);
	...
	auto shopIcon5 = ShopDefenseIcon::createDefenseIcon(4);
	...
	auto shopIcon6 = ShopDefenseIcon::createDefenseIcon(5);
	...
	auto shopIcon7 = ShopDefenseIcon::createDefenseIcon(6);
	...
	auto shopIcon8 = ShopDefenseIcon::createDefenseIcon(7);
	...
	auto shopIcon9 = ShopDefenseIcon::createDefenseIcon(8);
	...
	auto shopIcon10 = ShopDefenseIcon::createDefenseIcon(9);

...
#include "BaseTower.h"
//#include "TouchLayer.h"  咱们的鼠标层！

USING_NS_CC;

class GameManager//主要应该是用来存档 一盘游戏的全局变量等
{
public:
	void CreateGameManager(int slot);

	Vector<BaseMonster*> monsterBlueVector;//monster的两个vector是gamemanager下的全局变量啦！
	Vector<BaseMonster*> monsterRedVector;
	Vector<BaseTower*> towerBlueVector;
	Vector<BaseTower*> towerRedVector;
...
private:
	static GameManager * instance;//创建实例
};

...
Yase* Yase::createMonster(int color)//std::vector<Point> points, std::vector<std::vector<std::vector<Point>>> sonpath)
{
	...
	if (_monster && _monster->init())
	{
		...
		_monster->setForce(219);//攻击值
		_monster->setArmor(121);//防御值
		_monster->setScope(35);//近程30
...
void Yase::shoot()//必须点攻击才攻击 everytime
{
	...
	if (monsterDistance <= towerDistance && nearestMonster != NULL /*&& nearestMonster->getCurrHp() > 0*/)
	{
		...
		//Point shootVector = nearestMonster->baseSprite->getPosition() - this->getPosition();//两点间 留下了geParent
...
bool Yase::init()
{
	...
	BaseMonster::onEnter();
	baseSprite = Sprite::create("YSup.png");
...
cocos2d::Vec2 Yase::tileCoordFromPosition(cocos2d::Vec2 pos)
{
